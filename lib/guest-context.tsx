...
export function GuestProvider({ children }: { children: React.ReactNode }) {
  ...
  const calculateStats = useCallback((guestList: Guest[]): GuestStatistics => {
    ...
    guestList.forEach(guest => {
      ...
      stats.byConfirmationStage[stage] = (stats.byConfirmationStage[stage] || 0) + 1;
  ...
  const recordFieldUpdate = useCallback((guestId: string, fields: string[], timestamp: number) => {
  ...
  const isUpdateStale = useCallback((guestId: string, fields: string[], timestamp: number): boolean => {
      ...
      try {
        ...
        if (update.type === 'add' && data.guest) {
          ...
          setGuests(prev => prev.map(g => 
            g.id === update.guestId ? data.guest : g
          ));
        } else if (update.type === 'update' && data.guest && update.guestId && update.updatedFields) {
          // Check if this update is stale before applying
          if (!isUpdateStale(update.guestId, update.updatedFields, update.timestamp)) {
            ...
            setGuests(prev => prev.map(g => {
              ...
              Object.keys(data.guest).forEach(key => {
                if (update.updatedFields?.includes(key) && 
                    !isUpdateStale(update.guestId!, [key], update.timestamp)) {
                  (filteredUpdate as Record<string, unknown>)[key] = (data.guest as unknown as Record<string, unknown>)[key];
                }
              });
        ...
        updateQueue.current = updateQueue.current.filter(u => u.id !== update.id);
        ...
        if (update.previousState) {
          if (update.type === 'add') {
          ...
          } else if (update.type === 'update' && update.guestId && update.updatedFields) {
            ...
            setGuests(prev => prev.map(g => {
              ...
              update.updatedFields!.forEach(field => {
                (rollbackUpdate as Record<string, unknown>)[field] = (prevGuest as unknown as Record<string, unknown>)[field];
              });
            ...
            if (latestFieldUpdates.current[update.guestId]) {
              update.updatedFields.forEach(field => {
                if (latestFieldUpdates.current[update.guestId!][field] === update.timestamp) {
            ...
            setGuests(prev => {
              ...
              const insertIndex = Math.min(deletedGuest.display_order, newGuests.length);
  ...
  const addGuest = useCallback(async (name: string) => {
    ...
    updateQueue.current.push({
      ...
      newState: newGuest,
      apiCall: () => fetch(`/api/organizations/${organization.id}/guests`, {
        method: 'POST',
  ...
  const updateGuest = useCallback(async (guestId: string, updates: Partial<Guest>) => {
    ...
    updateQueue.current.push({
      ...
      newState: updates,
      updatedFields,
      apiCall: () => fetch(`/api/guests/${guestId}`, {
        method: 'PATCH',
        headers: { 'Content-Type': 'application/json' },
  ...
  const reorderGuests = useCallback(async (fromIndex: number, toIndex: number) => {
    ...
    updateQueue.current.push({
      ...
      newState: newGuests,
      apiCall: () => fetch(`/api/organizations/${organization.id}/guests/reorder`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
